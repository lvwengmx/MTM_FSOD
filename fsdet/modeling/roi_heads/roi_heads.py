...
#input_shape: {p2, p3, p4, p5, p6} 256 ---->
...
@ROI_HEADS_REGISTRY.register()
class StandardROIHeads(ROIHeads): # 预训练baseline
    """
...
class ContrastiveROIHeads(StandardROIHeads):
    def __init__(self, cfg, input_shape):
        ...
        self.mpnn = {"_use": 'true',
                     ...
                     "graph_params": {"sim_type": 'correlation', "thresh": 'no', "set_negative": 'hard'},
                     "gnn_params": {"pretrained_path": 'no', "red": 4, "cat": 0, "every": 0,
                                    "gnn": {"num_layers": 1, "aggregator": "add", "num_heads": 1, "mlp": 1,
                                            "dropout_mlp": 0.1, "norm1": 1, "norm2": 1, "res1": 1, "res2": 1,
                                            "dropout_1": 0.1, "dropout_2": 0.1, "mult_attr": 0},
                                    "classifier": {"neck": 1, "num_classes": 0, "dropout_p": 0.4, "use_batchnorm": 0}
                                    }
                     }

        self.GraphAttentionV2Layer = GNN(self.fc_dim, self.mlp_head_dim, mpnn_dev=map_location, mpnn_opts=self.mpnn)
    ...
    def _forward_box(self, features, proposals):
        box_features = self.box_pooler(features, [x.proposal_boxes for x in proposals]) # ROI Pooling (7, 7, 256. 1024)
        box_features = self.box_head(box_features)  # [None, FC_DIM] MLP.out_dim == (1024, 1024)
        pred_class_logits, pred_proposal_deltas = self.box_predictor(box_features)  # (1024, 21) (1024, 80) 初始化
        box_features_contrast = self.GraphAttentionV2Layer(box_features)  # (1024, 256)
