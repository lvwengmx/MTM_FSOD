...
def chamfer_distance(point_cloud1, point_cloud2):
    point_cloud1 = point_cloud1.view(point_cloud1.size(0), point_cloud1.size(1), -1)
    point_cloud2 = point_cloud2.view(point_cloud2.size(0), point_cloud2.size(1), -1)
    dis_matrix = torch.cdist(point_cloud1, point_cloud2, p=2)
    min_dist1, _ = torch.min(dis_matrix, dim=2)
    min_dist2, _ = torch.min(dis_matrix, dim=1)
...
class RPNOutputs(object):
    ...
    def losses(self):
        ...
        num_pos_anchors = (gt_objectness_logits == 1).sum().item()   # 正样本数目 pos
        num_neg_anchors = (gt_objectness_logits == 0).sum().item()   # 负样本数目 512-pos
