...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.backbone = build_backbone(cfg)   # ResNet+FPN
        # if self.training:
        ...
        pixel_std1 = torch.Tensor(cfg.MODEL.PIXEL_STD1).to(self.device).view(num_channels, 1, 1)
        crop_size = 256
        ...
        # self.normalizer2 = lambda x: (x - pixel_mean) / pixel_std1
        self.croped_image = lambda x: transform(x)
        # self.transform = transforms.Lambda(lambda x: transforms.F.rotate(x, angle))
        self.to(self.device)
    ...
    def forward(self, batched_inputs):
        ...
        # backbone FPN image{image_tensor维度不同}
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        images, images_crop_1, images_crop_2 = self.preprocess_image(batched_inputs)
        features = self.backbone(images.tensor)  # List of L, FPN features
...
class ProposalNetwork(nn.Module):
    def __init__(self, cfg):
        ...
        self.backbone, self.backbone_bylo = build_backbone(cfg)
        self.proposal_generator = build_proposal_generator(cfg, self.backbone.output_shape())
